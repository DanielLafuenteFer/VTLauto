...
class InterfazDatosSobrescribir (customtkinter.CTkFrame):
    ...
    def button_function_aceptar(self):
        if self.verificar_datos() == True:
            self.datos_json()
            self.mostrar_siguiente("progreso_rescribir")
    def verificar_datos(self):
        validos = False
        if Configuracion.Configuracion.excel_VL_sobrescribir == None:
            MessageBox.showerror("Error", "Error al introducir el documento VL") 
        elif Configuracion.Configuracion.nombre_VTL_sobrescrito_creado == None:
            MessageBox.showerror("Error", "Error al introducir el documento o nombre del VTL")
        elif Configuracion.Configuracion.ruta_guardado_sobrescribir == None:
            MessageBox.showerror("Error", "Error al introducir la ruta de guardado")
        elif Configuracion.Configuracion.numero_estacion_sobrescribir == None:
